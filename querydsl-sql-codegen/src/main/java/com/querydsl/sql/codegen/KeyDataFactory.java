	...
	public Map<String, InverseForeignKeyData> getExportedKeys(DatabaseMetaData md, String catalog, String schema, String tableName) throws SQLException {
		...
		try {
			while (foreignKeys.next()) {
				...
				// bugfix that generated too many fk links because our crappy db2 setup
				if (!"NK".equals(foreignSchemaName)) {
					System.out.println("ExportedKeys: Skipping schema: " + foreignSchemaName);
					continue;
				}
	...
	public Map<String, ForeignKeyData> getImportedKeys(DatabaseMetaData md, String catalog, String schema, String tableName) throws SQLException {
		...
		try {
			while (foreignKeys.next()) {
				...
				// bugfix that generated too many fk links because our crappy db2 setup
				if (!"NK".equals(parentSchemaName)) {
					System.out.println("ImportedKeys: Skipping schema: " + parentSchemaName);
					continue;
				}
